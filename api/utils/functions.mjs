...
export function normalizeSenseBoxData(data) {
    const tempArr = data?.properties?.sensors?.filter(sensor => sensor.title === 'Temperatur')
    return tempArr
}
...
export async function fetchAndNormalize() {
    ...
    const ids = ["5c21ff8f919bf8001adf2488", "5eba5fbad46fb8001b799786", "5ade1acf223bd80019a1011c"]
    const results = []
    for(const id of ids) {
        const res = await fetch(`https://api.opensensemap.org/boxes/${id}?format=geojson`);
